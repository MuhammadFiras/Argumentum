...
<head>
    ...
    <style>
        ...
        .best-answer-card {
            ...
            /* Border hijau pada card jawaban terbaik */
            background-color: #e6f7f0;
            /* Warna latar sedikit hijau untuk jawaban terbaik */
        }

        .toggle-best-answer-btn {
            /* Untuk tombol di dalam form */
            font-size: 0.85rem;
        ...
        <?php if (!empty($question)): ?>
            <div class="question-header">
                ...
                <div class="user-info mb-2 d-flex align-items-center">
                    <span class="me-1">Ditanyakan oleh:</span>
                    <a href="<?= base_url('profile/' . esc($question['id_user'])) ?>" class="d-flex align-items-center text-decoration-none text-dark">
                    ...
                    <span class="ms-1">· <?= CodeIgniter\I18n\Time::parse($question['created_at'])->humanize() ?></span>
                </div>
                <hr>
                ...
                <?php foreach ($answers as $answer): ?>
                    <div class="answer-card <?= $answer['is_best_answer'] ? 'best-answer-card' : '' ?>" id="answer-<?= esc($answer['id_answer']) ?>">
                        <div class="d-flex justify-content-between align-items-start">
                            <div class="user-info">
                                <a href="<?= base_url('profile/' . esc($answer['id_user'])) ?>" class="d-flex align-items-center text-decoration-none text-dark">
                        ...
                        <div class="d-flex justify-content-between align-items-center mt-3">
                            ...
                            <?php if (session()->get('isLoggedIn') && $question['id_user'] == session()->get('user_id')): ?>
                                <div class="ms-auto">
                                    <form action="<?= site_url('answer/toggle-best/' . $answer['id_answer']) ?>" method="post" class="d-inline">
                                        <?= csrf_field() ?>
                                        <?php if ($answer['is_best_answer']): ?>
